...
from .models import Field, get_default_model, get_model_from_spec
...
def build_filters(filter_spec):
    ...
    if _is_iterable_filter(filter_spec):
        return list(
            chain.from_iterable(build_filters(item) for item in filter_spec)
        )
...
def apply_filters(stmt: Select, filter_spec, do_auto_join=True):
    ...
    :returns:
        The :class:`sqlalchemy.sql.selectable.Select` instance after all the
